    ...
    <div class="gestionAll col-12 row my-5 w-100" id="arc_<?= $arc->id() ?>">
        <div class="btn-toolbar col-3" role="toolbar" aria-label="Toolbar with button groups">
            <div class="btn-group-vertical mr-2" role="group" aria-label="First group">
                <button type="button" class="btn" onclick="EventGestionArcEpisode.EventChooseEpisode('gestionArc')">Gestion Arc</button>
                <?php foreach ($episodesStrings as $episode):?>
                <?php if($numberArc == $episode->arc_id()){ ?>
                <button type="button" class="btn" onclick="EventGestionArcEpisode.EventChooseEpisode(<?= $episode->id() ?>)"><?= $episode->title() ?></button>
                <?php  } ?>
                <?php endforeach?>
                <button type="button" class="btn" onclick="EventGestionArcEpisode.EventChooseEpisode('addEpisode')">Nouvelle épisode</button>

            </div>
        </div>
        <div class="col-9 flex-column justify-content-center align-items-center" id="gestionArc">
            <form action="GestionArcEpisode" method="post" class="w-100">
        ...
        <div class="ajoutepisode col-9 justify-content-center align-items-center" id="addEpisode">
            <form action="GestionArcEpisode" method="post" class="w-100">
